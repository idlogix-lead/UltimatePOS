...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        // dd(auth()->user()->hasPermissionTo('customer.view',"web"));
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        if (auth()->user()->can('profit_loss_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'ProfitLossCustomReport']),
                                __('1 Profit / Loss Report'),
                                ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(3) == 'profitloss']
                            );
                            $sub->url(
