...
class ReportController extends Controller
{
     ...
     */
    public static function getProfit($by = null, $api = [])
    {
        // $business_id = request()->session()->get('user.business_id');
        // $permitted_locations = auth()->user()->permitted_locations();
        $api == [] ? $request = request():$request = $api;
        ...
        if (in_array($by, ['invoice'])) {
            $datatable->editColumn('gross_profit', function ($row) {
                ...
                $html = '<span class="gross-profit" data-orig-value="'.$profit.'" >'.$profit.'</span>';
            ...
            $datatable->editColumn(
                'gross_profit',
                function ($row) {
                    return '<span class="gross-profit" data-orig-value="'.$row->gross_profit.'">'.$row->gross_profit.'</span>';
                });
        }
        ...
        if ($by == 'date') {
            ...
            $datatable->editColumn('transaction_date2', '{{($transaction_date)}}');
        }
